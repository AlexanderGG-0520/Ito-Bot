...
- A card number is private to its owner. `/hand` and `/draw` reply ephemerally with an opaque card slot and number. `/declare` publishes a clue associated with one specific slot; stage 1 requires one declaration per player, stage 2 two, and stage 3 three. The card owner may revise a clue without creating another slot. Played and skipped cards no longer need declarations, and play is blocked until all currently held cards are declared.
